...
| `createPostTemplate` / `updatePostTemplate` / `deletePostTemplate` / `postTemplate(s)` | No | No | Buffer's template library duplicates what this whole repo already is: a markdown file is EV's template. Adding a second, Buffer-hosted template store is two sources of truth for the same job, which is exactly the anti-pattern this brain's lead-magnet doctrine already rejects elsewhere ("don't build a third document builder"). |
...
1. **`bfr metrics <post-id>`** (or fold into `show --full`) -- read
   ...
   `wiki/tracking/performance-log.md` from a CLI call instead of a manual
   look at LinkedIn. *Why it made the cut:* directly fills the
   `engagement_24h` field the `post-live` signal schema in AGENTS.md
