// buf is a command-line client for Buffer's GraphQL API
...
// (idea, which has no channel attached at all). See `buf --help` for which
// verb is which.
//
...
//
// <channel> is a channel id or a channel name from 'buf channels' (cached,
// case-insensitive, to .buf-channels.json next to the repo root by default
...
const usageText = `Usage: buf <command> [args]

  buf channels                            list channel ids, cache to .buf-channels.json
  buf idea   <file.md>                    ideas board, no channel attached -- never posts
  ...
  buf thread <channel> <file.md>          PUBLISHES LIVE -- '---' blocks become a thread, will post
  buf image  <channel> <file.md> <path>   PUBLISHES LIVE -- drive-upload, attach, queue, will post
...
<channel> is a channel id or a channel name from 'buf channels' (cached, case-insensitive).
